library(dplyr, warn.conflicts = FALSE, quietly = TRUE)
library(purrr, warn.conflicts = FALSE, quietly = TRUE)

# capture batches from the command line
# args <- commandArgs(trailingOnly = TRUE)
# batches <- as.integer(args[1]):as.integer(args[2])
batches <- 19:33

# Tipton Pusto design matrix cleaned - clean_design_mat.R
load("data/design_mat.Rdata")
load("data/to_test.RData")

#-----------------------------------------------------------
# Source the functions
#-----------------------------------------------------------

source("1_data_gen_study_1.R")
source("2_estimation_study_1.R")
source("3_performance_criteria.R")

#-----------------------------------------------------------
# Simulation Driver - should return a data.frame or tibble
#-----------------------------------------------------------

run_sim <- function(iterations, 
                    m, 
                    tau, 
                    rho, 
                    beta_type, 
                    batch,
                    R,
                    full_form = "X1 + X2 + X3 + X4 + X5", 
                    test_dat = to_test,
                    design_matrix = design_mat, 
                    Ftest_types = c("Naive-F","HTZ"),
                    CWB_types = c("CWB","CWB-adj"),
                    seed = NULL) {
  
  require(dplyr, warn.conflicts = FALSE, quietly = TRUE)
  require(purrr, warn.conflicts = FALSE, quietly = TRUE)
  require(mvtnorm, warn.conflicts = FALSE, quietly = TRUE)
  require(clubSandwich, warn.conflicts = FALSE, quietly = TRUE)
  require(tidyr, warn.conflicts = FALSE, quietly = TRUE)
  require(stringr, warn.conflicts = FALSE, quietly = TRUE)
  require(tibble, warn.conflicts = FALSE, quietly = TRUE)
  
  if (beta_type %in% c("B1", "B5")){
    
    test_dat <- test_dat %>%
      filter(str_detect(cov_test, "X1"))
    
  } else if(beta_type %in% c("C1", "C5")){
    
    test_dat <- test_dat %>%
      filter(str_detect(cov_test, "X2"))
    
  } else if(beta_type %in% c("D1", "D5")){
    
    test_dat <- test_dat %>%
      filter(str_detect(cov_test, "X3"))
    
  } else if(beta_type %in% c("E1", "E5")){
    
    test_dat <- test_dat %>%
      filter(str_detect(cov_test, "X4"))
    
  } else if(beta_type %in% c("F1", "F5")){
    
    test_dat <- test_dat %>%
      filter(str_detect(cov_test, "X5"))
    
  } 
  
  if (!is.null(seed)) set.seed(seed)

  
  results <-
    rerun(iterations, {
        
      # generate data ------------------------------------------------------------
      meta_data <- generate_rmeta(m = m, 
                                  tau = tau, 
                                  rho = rho,
                                  covs = design_matrix,
                                  beta_type = beta_type)
      
      # Fit full model on data --------------------------------------------------
      y <- meta_data$g
      v <- meta_data$var_g
      X <- model.matrix(as.formula(paste("g ~ ", full_form)), data = meta_data)
      cluster <- meta_data$study
      
      full_model <- robu_handmade(X = X, y = y, v = v, cluster = cluster)
      
      res <- tibble()
      
      # get naive and htz -------------------------------------------------------
      names(test_dat$indices_test) <- test_dat$cov_test
      
      if ("Naive-F" %in% Ftest_types) {
        cov_mat_cr1 <- vcovCR(full_model, type = "CR1", cluster = cluster)
        naive_res <- Wald_test(full_model, 
                               constraints = constrain_zero(test_dat$indices_test),
                               vcov = cov_mat_cr1,
                               test = "Naive-F", 
                               tidy = TRUE) %>%
          dplyr::select(cov_test = hypothesis, test, p_val)
        
        res <- bind_rows(res, naive_res)
      }
      
      if (any(c("HTZ","EDT") %in% Ftest_types)) {
        cov_mat_cr2 <- vcovCR(full_model, type = "CR2", cluster = cluster)
        Ftest_res <- Wald_test(full_model, 
                             constraints = constrain_zero(test_dat$indices_test),
                             vcov = cov_mat_cr2,
                             test = intersect(Ftest_types, c("HTZ","EDT")), 
                             tidy = TRUE) %>%
          mutate(p_val = ifelse(Fstat < 0, 1, p_val)) %>%  # added this to fix the htz p val issue with negative F stat
          dplyr::select(cov_test = hypothesis, test, p_val)
      
        res <- bind_rows(res, Ftest_res)
        
      }
      

      # cwb ---------------------------------------------------------------------
      if (any(c("CWB","CWB-adj","CWB-fix","CWB-adj-fix") %in% CWB_types)) {
        cwb_params <- test_dat %>%
          dplyr::select(null_model, indices_test)
        
        boot_res <- pmap_dfr(cwb_params, 
                             .f = cwb, 
                             dat = meta_data,
                             R = R,
                             full_form = full_form,
                             types = CWB_types) %>%
          bind_cols(dplyr::select(test_dat, cov_test)) %>%
          gather(test, p_val, -c(cov_test))
        
        res <- bind_rows(res, boot_res)
        
      }
      
    })  %>%
    bind_rows()
  
  calc_performance(results)
}

# demonstrate the simulation driver

#-------------------------------------
# Experimental Design
#-------------------------------------

# include design matrix, exclude to_test

set.seed(20201108) # change this seed value!

# now express the simulation parameters as vectors/lists

design_factors <- list(
  m = c(10, 20, 40, 80), 
  tau = c(0.1, 0.3),
  rho = c(0.5, 0.8),
  R = 399,
  beta_type = c("A", "B1", "B5", "C1", "C5", "D1", "D5", "E1", "E5", "F1", "F5"),
  batch = 1:80
)

# combine into a design set
params <-
  cross_df(design_factors) %>%
  mutate(
    iterations = 50, # change this to how many ever iterations
    seed = round(runif(1) * 2^30) + 1:n()
  )



# Just checking!! ---------------------------------------------------------

quick_params <- 
  params %>% 
  filter(batch %in% batches)

rm(design_factors, params)
source_obj <- ls()



#--------------------------------------------------------
# run simulations in parallel - mdply workflow
#--------------------------------------------------------

library(Pusto)

cluster <- start_parallel(source_obj = source_obj, 
                          setup = "register")



system.time(results <- plyr::mdply(quick_params, 
                                   .fun = run_sim, 
                                   Ftest_types = c("EDT"),
                                   CWB_types = c("CWB-fix","CWB-adj-fix"),
                                   .parallel = TRUE))

stop_parallel(cluster)



#--------------------------------------------------------
# Save results and details
#--------------------------------------------------------

session_info <- sessionInfo()
run_date <- date()

# batch names 
which_batches <- unique(quick_params$batch)
results_file <- paste0("sim_test_", paste(which_batches, collapse = "_"), ".RData")

# save
save(quick_params, results, session_info, run_date, file = results_file)
