...
calculate_F <- function(beta, vcov, constraints, test, p = 6){
  ...
  C_beta <- C_mat %*% beta
  Q <- as.numeric(t(C_beta) %*% chol2inv(chol(C_mat %*% vcov %*% t(C_mat))) %*% C_beta)
  q <- nrow(C_mat)
...
cwb <- function(null_model, 
  ...
  if (any(c("CWB-adj","CWB-adj-fix") %in% types)) {
    e_tilde_j <- split(dat$res, dat$study)
  ...
  bootstraps <- rerun(.n = R, {
    ...
    if ("CWB" %in% types) {
      ...
      res <- bind_rows(res, 
        calculate_F(beta = full_mod_cwb$coefficients, 
                         vcov = cov_mat_cwb, 
    ...
    if ("CWB-fix" %in% types) {
      full_mod_cwb <- update_robu(full_mod_org, y = new_t, fix_tau_sq = TRUE)
      cov_mat_cwb <- full_mod_cwb$vcov
      res <- bind_rows(res, 
                       calculate_F(beta = full_mod_cwb$coefficients, 
                                   vcov = cov_mat_cwb, 
    ...
    if ("CWB-adj" %in% types) {
      ...
      res <- bind_rows(res, 
                       calculate_F(beta = full_mod_cwb_adj$coefficients, 
                                   vcov = cov_mat_cwb_adj, 
                                   constraints = indices_test, 
                                   test = "CWB Adjusted"))
    }
  
    if ("CWB-adj-fix" %in% types) {
      full_mod_cwb_adj <- update_robu(full_mod_org, y = new_t_adj, fix_tau_sq = TRUE)
      cov_mat_cwb_adj <- full_mod_cwb_adj$vcov
      res <- bind_rows(res, 
                       calculate_F(beta = full_mod_cwb_adj$coefficients, 
                                   vcov = cov_mat_cwb_adj, 
                                   constraints = indices_test, 
                                   test = "CWB Adjusted fixed tau"))
    }
  ...
  p_boot <- 
    bootstraps %>%
    group_by(test) %>%
    summarise(p_val = mean(Fstat > org_F), .groups = "drop") %>%
    ungroup() %>%
    spread(test, p_val)
