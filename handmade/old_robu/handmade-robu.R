...
robu_handmade <- function(X, y, v, cluster, rho = .8) {
...
update_robu <- function(mod, y, fix_tau_sq = FALSE) {
...
calc_CR <- function(updated_mod, X){
  
  study <- updated_mod$cluster
  m <- length(unique(study))
  res <- updated_mod$residuals
  ...
  w_j <- data.frame(study, w_j = updated_mod$weights) %>%
    distinct(.) %>%
    pull(w_j)
  
  W_j <- pmap(list(w_j, I_j), mult_mat_const)
  W <- updated_mod$weights * diag(nrow(X))
